...
# Read a file with apostrophe words
appos <- read.csv("appos.txt",stringsAsFactors = FALSE)
...
readFile = function(filepath,fraction,seed) {
    ...
    # Cast to lower case
    lines <- tolower(lines)
    # Replace words with appostrophe
    lines <- mgsub(lines,appos$appos,appos$repl)
    # Remove numbers
...
readFileCleanOld = function(filepath,fraction,seed) {
    # Open as "rb" to avoid ^Z problem
    con = file(filepath, "rb")
    lines <- readLines(con,encoding = "UTF-8")
    close(con)
    set.seed(seed)
    lines <- sample(lines,length(lines)*fraction)
    # Cast to lower case and remove non relevant symbols
    # Letters, spaces and apostrophes are retained
    lines <- strip(lines)
...
#words <- words %>% anti_join(profanity) # 9951709 
...
# First separate bigrams into two words, then filter out stopwords and profanities
...
# There are many bigramss that only occur very rarely
